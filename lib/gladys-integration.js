...
class GladysIntegration extends EventEmitter {
   ...
   * @param {Array} transports - Entries: `{ external_id, transport }` (max 100).
   * @returns {Promise<object>} `{ success }`.
   * @example
  ...
  async publishTransports(transports) {
    ...
    const entries = transports.map((entry) => {
      ...
      return { device_external_id: deviceExternalId, transport: entry.transport };
    });
   ...
   * @param {string} type - Declared capture type: 'udp-broadcast' | 'mdns' | 'ssdp'.
   * @param {object} [options] - Options.
   * @param {number} [options.timeoutSeconds] - Scan duration in seconds (1-30).
   * @returns {Promise<Array>} Raw results — 'udp-broadcast':
   * `[{ source_ip, source_port, payload_base64 }]`; 'mdns':
   * `[{ name, host, addresses, port, txt }]`; 'ssdp': raw headers per responder.
   * @example
