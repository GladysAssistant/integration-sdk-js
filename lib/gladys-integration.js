...
class GladysIntegration extends EventEmitter {
  ...
  /**
   * @description Register the handler called when Gladys asks a TTS provider
   * integration (manifest `type: "tts"`, contract B.20) to synthesize a text
   * — the announcement of a scene, or the voice-assistant answer. Synthesize
   * and resolve the audio as a `<content_type>;base64,...` data-URI: it is
   * acked back as `data.audio`, and Gladys itself serves the bytes to the
   * speaker through a short-lived tokenized URL. `language` is a best-effort
   * hint ('fr'…): the user's language for the voice assistant, `null` in
   * scenes. Accepted content types: audio/mpeg, audio/wav, audio/ogg,
   * audio/aac; decoded audio ≤ 5 MB; `text` arrives already capped at 2000
   * characters by Gladys (truncation, not an error). The ack is awaited under
   * 30 s (not the standard 5 s) so a local synthesis on a small board fits;
   * throwing acks the command as failed with the error message.
   * @param {Function} callback - `(text, language) => Promise<string>`.
   * @example
   * gladys.onTtsSynthesize(async (text, language) => `audio/mpeg;base64,${(await synthesize(text, language)).toString('base64')}`);
   */
  onTtsSynthesize(callback) {
    this.handlers.ttsSynthesize = callback;
  }
  ...
  async _handleMessage(rawData, initial) {
    ...
    switch (type) {
      ...
      case EXTERNAL_INTEGRATION.CAMERA_GET_IMAGE:
        ...
        break;
      case EXTERNAL_INTEGRATION.TTS_SYNTHESIZE:
        await this._runCommand(
          'ttsSynthesize',
          payload,
          [payload.text, payload.language !== undefined ? payload.language : null],
          (audio) => this._mapTtsAudio(audio),
        );
        break;
  ...
  /**
   * @description Validate the data-URI resolved by the onTtsSynthesize
   * handler and map it into the `command-result` data (contract B.20):
   * `<content_type>;base64,...` → `{ audio }`. The bounds of the Gladys core
   * are enforced here so a bad audio fails with an explicit message on the
   * integration side, instead of an opaque "invalid TTS audio" on the Gladys
   * side: curated content type, decoded audio of 1 byte to 5 MB. Throwing
   * acks the command as failed.
   * @param {string} audio - Value resolved by the handler.
   * @returns {object} The `command-result` data: `{ audio }`.
   * @example
   * this._mapTtsAudio('audio/mpeg;base64,SUQz...');
   */
  _mapTtsAudio(audio) {
    if (typeof audio !== 'string') {
      throw new Error('onTtsSynthesize: the handler must resolve a "<content_type>;base64,..." audio string');
    }
