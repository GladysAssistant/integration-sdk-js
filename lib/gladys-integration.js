...
class GladysIntegration extends EventEmitter {
  ...
  /**
   * @description Register the handler called when Gladys asks a "movies"
   * integration for its upcoming releases (contract B.19). Resolve an array
   * of pivot movie entries: `id` (string/number) and `title` and
   * `releaseDate` (ISO string or Date) required per entry; `overview`,
   * `posterUrl`, `trailerUrl` and `sourceUrl` optional. An entry missing a
   * required field is dropped by the Gladys core rather than rejecting the
   * whole list, and `posterUrl`/`trailerUrl`/`sourceUrl` are only kept when
   * they parse as an `http:`/`https:` URL — they render unmodified as
   * `img src` / `a href` on the dashboard. The resolved array is acked back
   * as `data.movies` — awaited under 15 s (not the standard 5 s) so a fresh
   ...
   * @param {Function} callback - `(options) => Promise<Array<object>>`.
   * @example
   * gladys.onMoviesGetUpcoming(async ({ language, region, daysAhead }) => [
   *   { id: 42, title: 'A movie', releaseDate: '2026-08-01', posterUrl: 'https://example.com/poster.jpg' },
   * ]);
   */
  onMoviesGetUpcoming(callback) {
    this.handlers.moviesGetUpcoming = callback;
  }
  ...
  /**
   * @description Fetch the houses configured in Gladys, sorted by name
   * (contract C.3). Generic to every geo-dependent integration that owns
   * its own logic (weather, air quality, pollen, movie showtimes near a
   * house…): fetch once at startup/reconnection, coordinates change rarely
   * and there is no dedicated update event. Requires `location: true` in
   * the manifest (shown on the install screen) — a `403 GladysApiError`
   * otherwise. `latitude`/`longitude` are `null` when the house has not
   * been located; several houses may exist, handle both.
   * @returns {Promise<Array<{id: string, name: string, selector: string, latitude: number|null, longitude: number|null}>>} The houses.
   * @example
  ...
  async _handleMessage(rawData, initial) {
    ...
    switch (type) {
      ...
      case EXTERNAL_INTEGRATION.WEATHER_GET_IMAGE:
        ...
        break;
      case EXTERNAL_INTEGRATION.MOVIES_GET_UPCOMING:
        await this._runCommand('moviesGetUpcoming', payload, [payload.options], (movies) => ({ movies }));
        break;
