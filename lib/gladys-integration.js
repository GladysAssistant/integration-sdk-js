...
class GladysIntegration extends EventEmitter {
  ...
  /**
   * @description Send a Wake-on-LAN magic packet through the Gladys core
   * network namespace. Requires `network_wake: true` in the integration
   * manifest; the core enforces this permission server-side.
   * @param {object} options - Wake-on-LAN options.
   * @param {string} options.mac - Target MAC address.
   * @param {string} [options.address] - Destination IPv4 address (default: 255.255.255.255).
   * @param {number} [options.port] - UDP destination port (default: 9).
   * @param {number} [options.sourcePort] - UDP source port (default: 0, ephemeral).
   * @returns {Promise<object>} `{ success }`.
   * @example
  ...
  async wakeOnLan(options) {
    if (typeof options !== 'object' || options === null || Array.isArray(options)) {
      throw new Error('wakeOnLan: "options" must be an object');
    }

    if (typeof options.mac !== 'string' || options.mac.length === 0) {
      throw new Error('wakeOnLan: "mac" must be a non-empty string');
    }

    const validMacAddress =
      /^(?:[0-9a-fA-F]{12}|(?:[0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}|(?:[0-9a-fA-F]{2}-){5}[0-9a-fA-F]{2})$/;

    if (!validMacAddress.test(options.mac)) {
      throw new Error('wakeOnLan: "mac" must be a valid MAC address');
    }

    if (options.address !== undefined && (typeof options.address !== 'string' || options.address.length === 0)) {
      throw new Error('wakeOnLan: "address" must be a non-empty string');
    }

    if (options.address !== undefined && !net.isIPv4(options.address)) {
      throw new Error('wakeOnLan: "address" must be a valid IPv4 address');
    }
    ...
    return this.httpClient.post('/network/wake', options);
  }
