...
class GladysIntegration extends EventEmitter {
   ...
   * re-authenticates and resynchronizes. Exception: when Gladys refuses the
   * token (close code 4000) reconnection stops for good — a refused token is
   * revoked (the container is being recreated) and never becomes valid again;
   * connect() rejects if it is still pending at that point.
  ...
  _openWebSocket(initial) {
    ...
    ws.on('close', (code) => {
      ...
      if (code === INVALID_ACCESS_TOKEN_CLOSE_CODE) {
        // The token was refused: it is either misconfigured (dev outside
        // Docker with a bad JWT) or revoked (token_version bump — the
        // container is being recreated with a fresh token anyway). A refused
        // token never becomes valid again, so reconnecting is pointless:
        // stop, and reject connect() when it is still pending.
        this.shouldReconnect = false;
        debug('authentication refused by Gladys, not reconnecting');
