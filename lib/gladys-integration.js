...
class GladysIntegration extends EventEmitter {
   ...
   * `uv_index`, `sunrise`, `sunset`), `hours` (≤ 24), `days` (≤ 8) and
   * `alerts` (≤ 10, CAP-style `severity` + `event`). The resolved object is
   * acked back as `data.weather` — awaited under 15 s (not the standard 5 s)
   * so a fresh third-party API call fits — then normalized and bounded by the
   * Gladys core (unknown fields dropped, percentages clamped to 0-100,
   * unknown conditions coerced to 'unknown'). Throwing acks the command as
   * failed, and the Gladys provider loop falls through to the next provider.
