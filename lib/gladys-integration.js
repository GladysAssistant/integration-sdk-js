...
class GladysIntegration extends EventEmitter {
  ...
  /**
   * @description Register the handler called when Gladys routes an AI request
   * to an AI provider integration (manifest `type: "ai"`, contract B.19): the
   * chat, the intent router and the weekly digest all funnel through it when
   * the user selects the integration as their AI provider. Receives an
   * OpenAI-compatible chat completion request (`messages`, `tools`,
   * `tool_choice`, plus the Gladys-specific `purpose` and `categories` — the
   * `model` field is stripped by the core, picking one is the provider's
   * job). Resolve an OpenAI-compatible completion: an object carrying
   * `choices[0].message` (with `content` and/or `tool_calls`) — anything else
   * is acked as failed before reaching Gladys. The ack is awaited under 120 s
   * (not the standard 5 s) so reasoning models fit; throwing acks the command
   * as failed with the error message, and Gladys reports the provider
   * unavailable — never a silent fallback to another provider.
   * @param {Function} callback - `(request) => Promise<object>`.
   * @example
   * gladys.onAiChat(async (request) => callOpenAiCompatibleApi({ ...request, model: 'my-model' }));
   */
  onAiChat(callback) {
    this.handlers.aiChat = async (request) => this._mapAiChatCompletion(await callback(request));
  }
  ...
  async _handleMessage(rawData, initial) {
    ...
    switch (type) {
      ...
      case EXTERNAL_INTEGRATION.MESSAGE_SEND:
        ...
        break;
      case EXTERNAL_INTEGRATION.AI_CHAT:
        await this._runCommand('aiChat', payload, [payload.request]);
        break;
  ...
  /**
   * @description Validate the completion resolved by the onAiChat handler
   * before acking it (contract B.19, same check as the Gladys core): an
   * object carrying an object `choices[0].message`. Failing here acks the
   * command as failed with an explicit message instead of letting a malformed
   * completion travel to Gladys and die at the contract boundary.
   * @param {object} completion - Value resolved by the handler.
   * @returns {object} The completion, acked back as the `command-result` data.
   * @example
   * this._mapAiChatCompletion({ choices: [{ message: { content: 'Hi!' } }] });
   */
  _mapAiChatCompletion(completion) {
    const isObject = (value) => value !== null && typeof value === 'object' && !Array.isArray(value);
    if (!isObject(completion) || !Array.isArray(completion.choices) || !isObject(completion.choices[0]?.message)) {
      throw new Error(
        'onAiChat: the handler must resolve an OpenAI-compatible completion carrying an object choices[0].message',
      );
    }
    return completion;
  }
