...
class GladysIntegration extends EventEmitter {
   ...
   * fields carry the chosen device `external_id`). Resolve an object of the
   ...
   * carries the chosen device `external_id`), `language` the requesting
   ...
   *     { type: 'value', label: { en: 'Battery' }, device_feature: gladys.externalId(`${settings.vacuum}:battery`) },
   ...
   * can hang on a public wall panel) and `{ settings }`, the validated
   * instance settings. Resolve an optional message shown as a toast — a
   ...
   * @param {string} key - Widget key, as declared in the manifest.
   * @param {Function} callback - `(actionKey, params, { settings }) => Promise<string|object|void>`.
   * @example
    ...
    switch (type) {
      ...
      case EXTERNAL_INTEGRATION.WIDGET_ACTION:
        await this._runCommand(
          ...
          payload,
          [payload.action_key, payload.params, { settings: payload.settings }],
