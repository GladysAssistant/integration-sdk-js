 ...
 * environment variable (debug | info | warn | error | silent, default: info)
 * unless the `level` option pins it.
 *
 * This logger carries the INTEGRATION's own logs. It is unrelated to the SDK
 * internal debug channel (debug.js): the SDK itself still never logs anything
 * unless DEBUG=gladys-integration-sdk is set.
 */
...
const createLogger = (options = {}) => {
  ...
  const threshold = () => LOG_LEVELS[level || process.env.LOG_LEVEL] || LOG_LEVELS.info;
