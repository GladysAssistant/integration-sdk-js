...
function checkStatusComponent(raw, path, report) {
  ...
  // the core walks the whole array and keeps valid rows until the cap: an
  // invalid row never consumes a slot, a later valid one fills it
  let kept = 0;
  let extra = 0;
  raw.items.forEach((item, index) => {
    if (kept >= MAX_STATUS_ITEMS) {
      extra += 1;
      return;
    }
  ...
  if (extra > 0) {
    report.warn(
      `${path}.items`,
      `has ${kept + extra} rows past the invalid ones, ${MAX_STATUS_ITEMS} allowed: the extra rows are dropped`,
    );
  }
  if (kept === 0) {
...
function checkChartComponent(raw, path, report) {
  ...
  if (raw.device_features !== undefined) {
    ...
    raw.series.slice(0, MAX_CHART_SERIES).forEach((series, index) => {
      ...
      // like the status rows: valid points fill the cap, invalid ones never
      // consume a slot
      let keptPoints = 0;
      let invalidPoints = 0;
      let extraPoints = 0;
      series.points.forEach((point) => {
        if (keptPoints >= MAX_CHART_POINTS) {
          extraPoints += 1;
        } else if (isPlainObject(point) && isIsoDate(point.t) && isFiniteNumber(point.v)) {
          keptPoints += 1;
        } else {
          invalidPoints += 1;
        }
      ...
      if (extraPoints > 0) {
        report.warn(
          `${seriesPath}.points`,
          `has ${keptPoints + extraPoints} points past the invalid ones, ${MAX_CHART_POINTS} allowed: the extra ones are dropped`,
        );
      }
      if (keptPoints === 0) {
...
function checkCardListComponent(raw, path, report) {
  ...
  // like the status rows: valid items fill the cap, invalid ones never
  // consume a slot
  let kept = 0;
  let extra = 0;
  raw.items.forEach((item, index) => {
  ...
  if (extra > 0) {
    report.warn(
      `${path}.items`,
      `has ${kept + extra} items past the invalid ones, ${maxItems} allowed in "${display}": the extra ones are dropped`,
    );
  }
  if (kept === 0) {
...
// base64 encodes 3 bytes in 4 chars: a string longer than this cannot decode
// under the size cap, and the core refuses it before the decode allocates
const MAX_IMAGE_BASE64_LENGTH = Math.ceil(MAX_WIDGET_IMAGE_BYTES / 3) * 4 + 4;
...
function validateWidgetImage(rawBase64) {
  ...
  if (rawBase64.length > MAX_IMAGE_BASE64_LENGTH) {
    // refused before the decode allocates, like the core: the size is the
    // lower bound the base64 length guarantees
    const atLeastKb = Math.ceil((rawBase64.length * 3) / 4 / 1024);
    return [`image: at least ${atLeastKb} KB received, ${maxKb} KB allowed — resize it integration-side`];
  }
