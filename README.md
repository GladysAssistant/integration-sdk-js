...
| `getHouses()`                              | The houses configured in Gladys, sorted by name: `[{ id, name, selector, latitude, longitude }]`. Requires `location: true` in the manifest (403 otherwise). `latitude`/`longitude` are `null` when not located; several houses may exist. Generic to any geo-dependent integration owning its own logic (weather, air quality, movie showtimes near a house…) — fetch once at startup/reconnection, coordinates change rarely        |
...
| `onMoviesGetUpcoming(cb)`                                             | `(options) => Promise<Array<object>>` — movies integrations (manifest `type: "movies"`): `options = { language, region, daysAhead }`; resolve an array of pivot movie entries (`id`/`title`/`releaseDate` required, `overview`/`posterUrl`/`trailerUrl`/`sourceUrl` optional — the URL fields must be `http:`/`https:`), acked back as `data.movies` (awaited under 15 s, not 5 s, so a fresh third-party API call fits) |
