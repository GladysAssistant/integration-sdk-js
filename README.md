...
| `onTtsSynthesize(cb)`                                                 | `(text, language) => Promise<string>` — TTS provider integrations (`type: "tts"`): synthesize `text` (already capped at 2000 characters by Gladys) and resolve the audio data-URI (`audio/mpeg` \| `audio/wav` \| `audio/ogg` \| `audio/aac`, `;base64,...`, ≤ 5 MB decoded); acked back as `data.audio`, awaited under 30 s (not 5 s) so a local synthesis fits. `language` is a best-effort hint (`null` in scenes) |
...
### TTS provider (the voice of the instance)

Declare `type: "tts"` in the manifest and the integration becomes selectable as _the_ voice of the instance — a
self-hosted engine (Piper-like), a cloud TTS API… The user picks it in the "Voice (TTS provider)" card of the
Artificial Intelligence page of the Gladys UI; Gladys Plus stays the built-in default. Same screens as a
communication integration: **Configuration only** (no Devices/Discovery) — voice, model, speed, per-language
voices… all belong to the integration's own `config_schema`.

Gladys asks for a synthesis through `onTtsSynthesize` — the announcement of a scene, or the voice-assistant
answer — and serves the resolved audio to the speaker itself, through a short-lived tokenized URL: the
integration never handles the delivery.

```js
gladys.onTtsSynthesize(async (text, language) => {
  const mp3 = await synthesize(text, language); // your engine code, returns a Buffer
  return `audio/mpeg;base64,${mp3.toString('base64')}`;
});
```

- `text` arrives already capped at **2000 characters** by Gladys (truncation, not an error: a long AI answer
  speaks its beginning rather than failing).
- `language` is a best-effort hint (`'fr'`…): the user's language for the voice assistant, `null` in scenes.
- Resolve a `<content_type>;base64,...` data-URI — `audio/mpeg` | `audio/wav` | `audio/ogg` | `audio/aac`
  (what Sonos/Cast/browsers actually play), decoded audio ≤ **5 MB**. The SDK enforces these bounds before
  acking, so a bad audio fails with an explicit message on the integration side.
- The ack is awaited under **30 s** instead of the standard 5 s, so a local synthesis on a small board fits.
