...
const waitForCall = () => {
  ...
  return { called, resolve: (value) => resolveCall(value) };
};
...
describe('lifecycle events (device-created/updated/deleted, config-updated)', () => {
  ...
  it('should add the device to gladys.devices and call onDeviceCreated', async () => {
    const { called, resolve } = waitForCall();
    ...
    expect(received).to.deep.equal(device);
    expect(gladys.devices).to.deep.equal([device]);
  });
  ...
  it('should replace the device in gladys.devices and call onDeviceUpdated', async () => {
    ...
    await called;
    expect(gladys.devices).to.deep.equal([updated]);
  });
  ...
  it('should update gladys.config and call onConfigUpdated with the complete values', async () => {
    ...
    const received = await called;
    expect(received).to.deep.equal({ latitude: 43.6, unit: 'celsius' });
    expect(gladys.config).to.deep.equal({ latitude: 43.6, unit: 'celsius' });
  });
  ...
  it('should ignore lifecycle events silently when no handler is registered', async () => {
    ...
    expect(gladys.devices).to.deep.equal([{ external_id: 'ext:ext-demo:x' }]);
    expect(gladys.config).to.deep.equal({ a: 1 });
  });
